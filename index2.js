...
function initMap(){
 ...
 function getUserCoordinates(){
        ...
        map.setCenter(39.510139,-103.724731);
  ...
  //Map Options
  var options = {
    center: { lat: 39.510139, lng: -103.724731 },
    zoom: 5,
  ...
  function addMarker(coords, name, address, staffed, available, phone, percent, hospital, website){
    ...
    if( available < 0)
    {
      ...
      if(available<0){
        available = Math.floor(Math.random() * staffed + 1);
      }
    }
      var icon = {
  ...
  autocomplete.addListener('place_changed', function() {
    ...
    if (place.geometry.viewport) {
      ...
      map.setZoom(zoomConstant);  // Why 17? Because it looks good.
    }
